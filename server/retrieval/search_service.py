...
def improve_search_query(
    stock_no: str,
    role: Literal["SQAA_AGENT"] = "SQAA_AGENT",
) -> List[str]:

    template = "'{stock_no}'에 대해 {perspective} 웹검색에 적합한 3개의 검색어를 제안해주세요. 각 검색어는 25자 이내로 작성하고 콤마로 구분하세요. 검색어만 제공하고 설명은 하지 마세요."

    perspective_map = {
        "SQAA_AGENT": "STT 문장을 표준 스크립트와 매칭하여, 각 항목에 따라 평가를 진행하십시오. 각 항목별 검출 사유 및 개선 코멘트도 작성해주세요."
    }

    prompt = template.format(stock_no=stock_no, perspective=perspective_map[role])
